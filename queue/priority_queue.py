...
A priority queue is a queue where the most important element is always at the
front.
...
Downside: inserting new items is slow because they must be inserted in sorted
order.

As a balanced binary search tree. This is great for making a double-ended
priority queue because it implements both "find minimum" and "find maximum"
efficiently.

As a heap. The heap is a natural data structure for a priority queue.
In fact, the two terms are often used as synonyms.
A heap is more efficient than a sorted array because a heap only has to be
partially sorted.
